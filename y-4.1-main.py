...
openai.api_base = "https://api.chatanywhere.com.cn/v1"#指定api url
# openai.api_base = "https://api.chatanywhere.cn/v1"
messages = []#一个列表，存放聊天上下文，建议长度最大为5
openai.api_key="sk-rzbJXmWJOMOdQ7PgHehAT82f6UJNmioBkSiz5xz0tPCGj3Oz"#key,去https://github.com/chatanywhere/GPT_API_free 获得

sentry_sdk.init(
    dsn="https://89888d799d5eeb0b312495101590cf23@o4506008195956736.ingest.sentry.io/4506008234688512",#故障汇报sdk，可删除此函数和库
...
b=0#启动次数
lingpai='50357763a9034c07c3a6589d5fsa413ce2b0f8d65a523b084d6de33bd62ba569f1gds54435sd4fgv53s397d687a'#机器人Token
for s in range(30):#最大重试次数
    try:
        zdsxw=6#最大上下文长度
        system_message='使用中文回答问题，如果用户输入了其他语言，则使用其他语言回复'#自定义提示
        messages.append({"role":"system","content":system_message})#代表系统消息
        b+=1#启动次数+1
        url='https://a1.fanbook.mobi/api/bot/'+lingpai+'/sendMessage'#日志频道，可忽略
        ...
        def get_audio_duration(url,msg):#为了节省时间，已简化获取时间函数
            """
            try:
                # 要下载的文件的URL
                file_url = url
            ...
            return len(msg) // 4
        ms='0'#存放模式，已弃用
        pdid=433212507046281216#指定频道id，已弃用
        sycyid=[]#使用成员id
        cysycs=[]#成员使用次数
        jgczsj=0#警告重置时间
        gjc=''#绘图关键词
        ...
        def on_message(ws, message):
            ...
            if message["action"] =="push":
                if message["data"]["author"]["bot"] == false:
                    ...
                    if "${@!469790724255502336}" in content['text']:#非常重要，填仅@机器人的消息示例，格式:${@!机器人id}，机器人收到消息但是不尝试回复请注意这里
                        if zdyzyxx:
                        ...
                        if int(cysycs[sycyid.index(userid)]) == 7:
                            ...
                            "text": '速率限制：\n你当前给机器人发送消息数超过每两分钟6次，请休息一下，2分钟后再来吧'+get_ad(adjl=2),#每2分钟限制，防止刷api
                            "reply_to_message_id":int(message["data"]["message_id"])
                            })
                            print(jsonfile)
                            postreturn=requests.post(url,data=jsonfile,headers=headers)
                            colorize_json(smg2=postreturn.text,pcolor='d')
                        elif int(cysycs[sycyid.index(userid)]) < 7:#如果小于7次，则回复
                            if '模式切换' in content['text']:
                                if mxlb[fwqlb.index(fwqid)] == 'ChatGPT':
                                    ...
                                    jsonfile=json.dumps({
                                    "chat_id":int(message["data"]["channel_id"]),
                                    "text": '抱歉，暂时只有ChatGPT支持模式切换，其他均为文本输出，请切换模型为ChatGPT再切换模式'+get_ad(adjl=2),
                                    "reply_to_message_id":int(message["data"]["message_id"])
                                    })
                                    print(jsonfile)
                                    postreturn=requests.post(url,data=jsonfile,headers=headers)
                                ...
                                jsonfile=json.dumps({
                                "chat_id":int(message["data"]["channel_id"]),
                                "text": '当前运行节点信息：\n运行节点名：云服务器1[公用]\nip:1.117.76.68\n参考位置：中国-上海市 腾讯云\n近期累计调用次数：'+str(dycs)+'次\n2分钟内调用次数：'+str(efzdy)+'次\n版本号：4.1\n新功能体验/反馈，欢迎前往：https://fanbook.mobi/LmgLJF3N'+get_ad(adjl=2),
                                "reply_to_message_id":int(message["data"]["message_id"])
                                })
                                print(jsonfile)
                                postreturn=requests.post(url,data=jsonfile,headers=headers)
                                ...
                                if 'ChatGPT' in content['text']:
                                    ...
                                    jsonfile=json.dumps({
                                    "chat_id":int(message["data"]["channel_id"]),
                                    "text":"模型已切换为星火大模型V2.0"+get_ad(adjl=2),
                                    "reply_to_message_id":int(message["data"]["message_id"])
                                    })
                                    print(jsonfile)
                                    postreturn=requests.post(url,data=jsonfile,headers=headers)
                                    ...
                                    jsonfile=json.dumps({
                                    "chat_id":int(message["data"]["channel_id"]),
                                    "text":"找不到你选择的模型，请重新选择"+get_ad(adjl=2),
                                    "reply_to_message_id":int(message["data"]["message_id"])
                                    })
                                    print(jsonfile)
                                    postreturn=requests.post(url,data=jsonfile,headers=headers)
