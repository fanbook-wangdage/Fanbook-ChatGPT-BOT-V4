...
#openai.api_base = "https://api.chatanywhere.com.cn/v1"#两个api均可选
openai.api_base = "https://openkey.cloud/v1"

messages = []
openai.api_key="sk-XTLKHYnZTrWdR1Ys8VYhIC9Ljii5kR8SiCDD6T5fV8"#https://openkey.cloud/
...
b=0
lingpai='50357763a9034c07c3a6cds53af43sa0f8d65a523b0vfds52145dsv69f1397d687a992731ae59b8e16911c3319a7'
for s in range(30):
    try:
        zdsxw=6
        b+=1
                    ...
                    if "${@!469790724255502336}" in content['text']:
                        ...
                        if int(cysycs[sycyid.index(userid)]) == 7:
                            ...
                            if '模式切换' in content['text']:
                                ...
                                postreturn=requests.post(url,data=jsonfile,headers=headers)
                                colorize_json(smg2=postreturn.text,pcolor='d')
                            elif 'system_runpy' in content['text']:
...
"""
                                    if mxlb[fwqlb.index(fwqid)] == '星火大模型V2.0' or mxlb[fwqlb.index(fwqid)] == '文心一言':
                                        ...
                                        jsonfile=json.dumps({
                                        "chat_id":int(message["data"]["channel_id"]),
                                        "text": text+get_ad(adjl=8,fwqid=fwqid),
                                        #"parse_mode":"Fanbook",
                                        "reply_to_message_id":int(message["data"]["message_id"])
                                        })
                                        print(jsonfile)
